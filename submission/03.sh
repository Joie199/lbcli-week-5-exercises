# What is the address of the P2SH script wrapping the witness redeem script in this transaction
transaction=""

decoded=$(bitcoin-cli -regtest decoderawtransaction "$transaction")

redeemScript=$(echo "$decoded" | jq -r '.vin[0].scriptSig.asm' | awk '{print $NF}')

scriptInfo=$(bitcoin-cli -regtest decodescript "$redeemScript")

address=$(echo "$scriptInfo" | jq -r '.segwit."p2sh-segwit"')

echo "$address"