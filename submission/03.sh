# What is the address of the P2SH script wrapping the witness redeem script in this transaction
transaction=""
# 1. extract FULL witness program (do NOT modify)
redeem=$(bitcoin-cli -regtest decoderawtransaction "$transaction" \
  | jq -r '.vout[0].scriptPubKey.hex')

# 2. HASH160(redeemScript)
hash160=$(echo -n "$redeem" \
  | xxd -r -p \
  | openssl dgst -sha256 -binary \
  | openssl dgst -rmd160 \
  | awk '{print $2}')

# 3. build P2SH scriptPubKey
spk="a914${hash160}87"

# 4. decode to address
address=$(bitcoin-cli -regtest decodescript "$spk" | jq -r '.p2sh')

echo -n "$address"