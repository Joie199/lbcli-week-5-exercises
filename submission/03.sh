# What is the address of the P2SH script wrapping the witness redeem script in this transaction
transaction=""
witness_script=$(bitcoin-cli -regtest decoderawtransaction "$transaction" | jq -r '.vin[0].txinwitness[-1]')
sha256=$(echo -n "$witness_script" | xxd -r -p | openssl dgst -sha256 -binary | xxd -p -c 256 | tr -d '\n')
program="0020${sha256}"
hash160=$(echo -n "$program" | xxd -r -p | openssl dgst -sha256 -binary | openssl dgst -rmd160 | awk '{print $2}')
spk="a914${hash160}87"
address=$(bitcoin-cli -regtest decodescript "$spk" | jq -r '.p2sh // .address // .addresses[0]')

echo "$address"