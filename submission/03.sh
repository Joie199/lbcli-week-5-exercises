# What is the address of the P2SH script wrapping the witness redeem script in this transaction
transaction=""
decoded=$(bitcoin-cli -regtest decoderawtransaction "$transaction")

scriptSig=$(bitcoin-cli -regtest decoderawtransaction "$transaction" | jq -r '.vin[0].scriptSig.hex')
descriptor="sh(raw(${scriptSig:2}))"
address=$(bitcoin-cli -regtest deriveaddresses "$(bitcoin-cli -regtest getdescriptorinfo "$descriptor" | jq -r '.descriptor')" | jq -r '.[0]')
echo "$address"