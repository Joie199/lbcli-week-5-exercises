# What is the address of the P2SH script wrapping the witness redeem script in this transaction
transaction=""
# ONLY source of truth: scriptPubKey
witness=$(bitcoin-cli -regtest decoderawtransaction "$transaction" \
  | jq -r '.vout[0].scriptPubKey.hex')

# treat it directly as redeemScript (no modification)
redeem="$witness"

# HASH160(redeemScript)
hash160=$(printf "%s" "$redeem" \
  | xxd -r -p \
  | openssl dgst -sha256 -binary \
  | openssl dgst -rmd160 \
  | awk '{print $2}')

# build P2SH scriptPubKey
spk="a914${hash160}87"

# decode address
bitcoin-cli -regtest decodescript "$spk" | jq -r '.p2sh'