# What is the address of the P2SH script wrapping the witness redeem script in this transaction
transaction=""
# 1. Extract witness script from scriptSig (last push)
witness_script=$(bitcoin-cli -regtest decoderawtransaction "$transaction" \
  | jq -r '.vin[0].scriptSig.asm' \
  | awk '{print $NF}')

# 2. SHA256
sha256=$(echo -n "$witness_script" \
  | xxd -r -p \
  | openssl dgst -sha256 -binary \
  | xxd -p -c 256)

# 3. Build redeemScript (P2WSH)
redeem="0020${sha256}"

# 4. HASH160
hash160=$(echo -n "$redeem" \
  | xxd -r -p \
  | openssl dgst -sha256 -binary \
  | openssl dgst -rmd160 \
  | awk '{print $2}')

# 5. Build P2SH scriptPubKey
spk="a914${hash160}87"

# 6. Get address
address=$(bitcoin-cli -regtest decodescript "$spk" | jq -r '.p2sh')

echo -n "$address"