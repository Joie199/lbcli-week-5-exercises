...
witness_hash=$(echo -n "$REDEEM_SCRIPT" | xxd -r -p | openssl dgst -sha256 | sed 's/^.* //' | tr -d '\n')
redeem_wrapped="0020${witness_hash}"
hash=$(echo -n "$redeem_wrapped" | xxd -r -p | openssl dgst -sha256 -binary | openssl dgst -rmd160 | sed 's/^.* //' | tr -d '\n')
spk="a914${hash}87"
