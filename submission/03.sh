# What is the address of the P2SH script wrapping the witness redeem script in this transaction
transaction=""
pubkeyhash=$(bitcoin-cli -regtest decoderawtransaction "$transaction" \
  | jq -r '.vout[0].scriptPubKey.hex' \
  | cut -c5-44)

# 2. Build redeem script for P2SH-P2WPKH
redeem="0014${pubkeyhash}"

# 3. Hash redeem script (HASH160)
hash160=$(echo -n "$redeem" \
  | xxd -r -p \
  | openssl dgst -sha256 -binary \
  | openssl dgst -rmd160 \
  | awk '{print $2}')

# 4. Build scriptPubKey (P2SH)
spk="a914${hash160}87"

# 5. Decode into address
address=$(bitcoin-cli -regtest decodescript "$spk" | jq -r '.p2sh')
echo "$address"