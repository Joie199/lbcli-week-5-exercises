# What is the address of the P2SH script wrapping the witness redeem script in this transaction
transaction=""
decoded=$(bitcoin-cli -regtest decoderawtransaction "$transaction")

scriptSig=$(echo "$decoded" | jq -r '.vin[0].scriptSig.hex')

redeem_script=${scriptSig:2}

descriptor="sh(raw(${redeem_script}))"

descriptor=$(bitcoin-cli -regtest getdescriptorinfo "$descriptor" | jq -r '.descriptor')

address=$(bitcoin-cli -regtest deriveaddresses "$descriptor" | jq -r '.[0]')

echo "$address"