# What is the address of the P2SH script wrapping the witness redeem script in this transaction
transaction=""
script=$(bitcoin-cli -regtest decoderawtransaction "$transaction" | jq -r '.vin[0].scriptSig.asm' | awk '{print $NF}')
hash=$(echo -n "$script" | xxd -r -p | openssl dgst -sha256 -binary | openssl dgst -rmd160 | awk '{print $2}' )
spk="a914${hash}87"
address=$(bitcoin-cli -regtest decodescript "$spk" | jq -r '.p2sh')
echo "$address"