# What is the address of the P2SH script wrapping the witness redeem script in this transaction
transaction=""
pubkeyhash=$(bitcoin-cli -regtest -named decoderawtransaction "$transaction" | jq -r '.vout[0].scriptPubKey.hex' | cut -c5-44)

redeem="0014${pubkeyhash}"

hash160=$(echo -n "$redeem" | xxd -r -p | openssl dgst -sha256 -binary | openssl dgst -rmd160 | awk '{print $2}')

spk="a914${hash160}87"

address=$(bitcoin-cli decodescript "$spk" | jq -r '.p2sh')

echo -n "$address"