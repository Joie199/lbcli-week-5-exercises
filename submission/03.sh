# What is the address of the P2SH script wrapping the witness redeem script in this transaction
transaction=""

txid=$(bitcoin-cli -regtest decoderawtransaction "$transaction" | jq -r '.vin[0].txid')
vout=$(bitcoin-cli -regtest decoderawtransaction "$transaction" | jq -r '.vin[0].vout')


address=$(bitcoin-cli -regtest getrawtransaction "$txid" true | jq -r ".vout[$vout].scriptPubKey.address")

echo "$address"