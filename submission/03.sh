# What is the address of the P2SH script wrapping the witness redeem script in this transaction
transaction=""
# Extract witness script
REDEEM_SCRIPT=$(bitcoin-cli -regtest decoderawtransaction "$transaction" | jq -r '.vin[0].txinwitness[-1]')

# SHA256 of the witness script - clean output
witness_hash=$(echo -n "$REDEEM_SCRIPT" | xxd -r -p | openssl dgst -sha256 | awk '{print $NF}')

# Create the 22-byte redeem script
redeem_wrapped="0020${witness_hash}"

# HASH160 of the wrapped script - clean output
hash=$(echo -n "$redeem_wrapped" | xxd -r -p | openssl dgst -sha256 -binary | openssl dgst -rmd160 | awk '{print $NF}')

# Build the scriptPubKey
spk="a914${hash}87"

# Decode specifically for the P2SH address
SCRIPT=$(bitcoin-cli -regtest decodescript "$spk" | jq -r '.p2sh')

echo "$SCRIPT"