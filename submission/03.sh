# What is the address of the P2SH script wrapping the witness redeem script in this transaction
transaction=""
REDEEM_SCRIPT=$(bitcoin-cli -regtest decoderawtransaction "$transaction" | jq -r '.vin[0].scriptSig.asm' | awk '{print $NF}')
hash=$(echo -n "$REDEEM_SCRIPT" | xxd -r -p | openssl dgst -sha256 | awk '{print $2}')
HASH160=$(echo -n "0020$hash" | xxd -r -p | openssl dgst -sha256 -binary | openssl dgst -rmd160 | awk '{print $2}')
spk="a914${HASH160}87"
SCRIPT=$(bitcoin-cli -regtest decodescript "$spk" | jq -r '.p2sh')
echo "$SCRIPT"
